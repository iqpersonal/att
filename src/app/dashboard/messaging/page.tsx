...
export default function MessagingDashboard() {
    ...
    const [lastDocStudents, setLastDocStudents] = useState<QueryDocumentSnapshot | null>(null);
    const [lastDocLeads, setLastDocLeads] = useState<QueryDocumentSnapshot | null>(null);
    ...
    const fetchStudents = async (reset = true) => {
        ...
        try {
            ...
            if (!reset && lastDocStudents) {
                constraints.push(startAfter(lastDocStudents));
            }
            ...
            if (itemsToShow.length > 0) {
                setLastDocStudents(itemsToShow[itemsToShow.length - 1]);
            }
        } catch (error) {
    ...
    const fetchLeads = async (reset = true) => {
        ...
        try {
            ...
            if (!reset && lastDocLeads) {
                constraints.push(startAfter(lastDocLeads));
            }
            ...
            if (itemsToShow.length > 0) {
                setLastDocLeads(itemsToShow[itemsToShow.length - 1]);
            }
        } catch (error) {
        ...
        try {
            for (const recipient of recipients) {
                ...
                await addDoc(collection(db, "tenants", tenantId, "message_history"), {
                    toName: name,
                    toNumber: phone,
                    message: previewText.replace(/{{1}}/g, name),
                    ...
                    <section className="bg-white rounded-[40px] border border-slate-100 shadow-sm p-10 space-y-10">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative group">
                                ...
                                <input type="text" placeholder={`Search ${source}...`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-16 pr-8 py-5 bg-slate-50 border border-slate-50 rounded-[32px] focus:outline-none focus:ring-4 focus:ring-primary/5 focus:bg-white focus:border-primary/20 transition-all font-bold text-slate-900" />
                            </div>
                            ...
                            {(source === "students" ? hasMoreStudents : hasMoreLeads) && (
                                <div className="flex justify-center pt-4">
                                    <button onClick={() => source === "students" ? fetchStudents(false) : fetchLeads(false)} disabled={loadingMore} className="px-4 py-2 bg-white text-primary border border-primary/20 rounded-xl font-bold text-xs flex items-center gap-2 hover:bg-primary/5 transition-all disabled:opacity-50">
                                        {loadingMore ? (
                                            <>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {allPlaceholderIds.map((p) => (
                                        <div key={p} className="space-y-3">
                                            ...
                                            <input type="text" value={templateVariables[p] || ""} onChange={(e) => setTemplateVariables({ ...templateVariables, [p]: e.target.value })} placeholder={`Value for {{${p}}}`} className="w-full px-6 py-4 bg-slate-50 border border-slate-50 rounded-2xl focus:outline-none focus:ring-4 focus:ring-primary/5 focus:bg-white focus:border-primary/20 transition-all font-bold text-slate-900" />
                                        </div>
                                    ))}
                                </div>
                            </div>
